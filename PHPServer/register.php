...
$sql_max = "select max(uid) from user";
// 执行查找语句
...
// 插入新用户
$sql_update = "insert into user(uid,username,password) values(".$new_id.",'".$username."','".$password."');";
// 判断用户是否已存在
...
/////////////////验证结果并返回响应结果/////////////////
if ($sql_update_res) {
	$array = array('code' => 0, 'msg' => 'register sucesss!', 'data' => strval($new_id));
